...
inline FlashStringHelper asFlashStringHelper(const uint8_t * flashString) noexcept
{
	return static_cast<FlashStringHelper>(static_cast<const void *>(flashString));
}
...
inline FlashStringHelper readFlashStringPointer(const uint8_t * const * flashString) noexcept
{
	return static_cast<FlashStringHelper>(pgm_read_ptr(flashString));
}
